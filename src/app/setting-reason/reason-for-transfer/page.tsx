...
import { useReferralReasonStore } from "@/stores/referralReasonStore";
...
/* ── ReferType options (ใช้กับประเภท) matching Nuxt enum ── */
const referTypeOptions = [
  { value: "การปฏิเสธ", name: "การปฏิเสธ" },
  { value: "การยืนยันนัดหมาย", name: "การยืนยันนัดหมาย" },
  { value: "การเปลี่ยนแปลงนัดหมาย", name: "การเปลี่ยนแปลงนัดหมาย" },
  { value: "ยกเลิก", name: "ยกเลิก" },
  { value: "รับเข้ารักษา", name: "รับเข้ารักษา" },
  { value: "นัดรักษาต่อเนื่อง", name: "นัดรักษาต่อเนื่อง" },
  { value: "สิ้นสุดการส่งตัว", name: "สิ้นสุดการส่งตัว" },
  { value: "ทุกสถานะ", name: "ทุกสถานะ" },
];
...
export default function ReasonForTransferPage() {
  const reasonStore = useReferralReasonStore();
  ...
  const doFetch = useCallback(async (
    ...
    try {
      ...
      const res = await reasonStore.findAndCountReferralReason(params);
      setItems(res?.referralStatusDetails || []);
      setTotalCount(res?.totalCount ?? 0);
    } catch (err) {
      setError("ไม่สามารถโหลดข้อมูลเหตุผลได้");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleActive = async (item: any) => {
    ...
    try {
      await reasonStore.updateReferralReason(item.id, { isActive: !item.isActive });
    } catch (err) {
  ...
  const validateAddForm = () => {
    ...
    if (!addForm.name.trim()) errs.name = "กรุณากรอกชื่อเหตุผล";
    if (!addForm.statusType) errs.statusType = "กรุณาเลือกใช้กับประเภท";
  ...
  const handleAddSave = async () => {
    ...
    try {
      ...
      await reasonStore.createReferralReason(data);
      ...
      if (msg.toLowerCase().includes("already exists")) {
        setAddErrors({ name: "ชื่อเหตุผลนี้มีอยู่แล้ว" });
      }
    } finally {
  ...
  const validateEditForm = () => {
    ...
    const errs: Record<string, string> = {};
    if (!editForm.name.trim()) errs.name = "กรุณากรอกชื่อเหตุผล";
    if (!editForm.statusType) errs.statusType = "กรุณาเลือกใช้กับประเภท";
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      await reasonStore.updateReferralReason(editForm.id, data);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setModalLoading(true);
      await reasonStore.updateReferralReason(deletingItem.id, { isDelete: true });
  ...
  const renderReasonFormFields = (
    ...
    <>
      ...
      {/* Section: ข้อมูลเหตุผล */}
      <Box sx={{ border: "1px solid #e5e7eb", borderRadius: 2 }}>
        <SectionHeader title="ข้อมูลเหตุผล" />
        <Box sx={{ p: 2.5, display: "flex", flexDirection: "column", gap: 2 }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr 1fr" }, gap: 2 }}>
            {/* ชื่อเหตุผล */}
            <Box>
              <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>
                ชื่อเหตุผล <span style={{ color: "red" }}>*</span>
              </Typography>
              <TextField
                fullWidth
                size="small"
                placeholder="ชื่อเหตุผล"
                value={form.name}
            ...
            <Box>
              ...
              <Select
                ...
                onChange={(e) => setForm((prev: any) => ({ ...prev, statusType: e.target.value }))}
                sx={selectSx}
                MenuProps={dropdownMenuProps}
                error={!!errors.statusType}
              >
                <MenuItem value="">
                  <em style={{ color: "#9ca3af" }}>เลือกประเภทการส่งตัว</em>
                </MenuItem>
                {referTypeOptions.map((o) => (
                  <MenuItem key={o.value} value={o.value}>
                    {o.name}
                  </MenuItem>
                ))}
              </Select>
              {errors.statusType && <Typography color="error" variant="caption">{errors.statusType}</Typography>}
            </Box>
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ bgcolor: "#fff", border: "1px solid #e5e7eb", borderRadius: 2, p: 2.5, mb: 0 }}>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <TextField
            fullWidth
            size="small"
            placeholder="ค้นหาเหตุผลการส่งต่อ"
            value={search}
        ...
        {/* Filters row 1: Zone, Type, Hospital, ใช้กับประเภท */}
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr", lg: "1fr 1fr 1fr 1fr" }, gap: 2, mb: 2 }}>
          ...
          {/* ใช้กับประเภท (ReferType) */}
          <Box>
            <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>ใช้กับประเภท</Typography>
            <Select
              fullWidth
              size="small"
              displayEmpty
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: "#036245" }}>
                {["ลำดับ", "ชื่อเหตุผล", "สถานพยาบาล", "ใช้กับประเภท", "สำหรับผู้ป่วย", "การใช้งาน", "จัดการข้อมูล"].map(
                  (header) => (
              ...
              {loading ? (
                ...
                items.map((item, idx) => (
                  <TableRow key={item.id} hover sx={{ "&:nth-of-type(even)": { bgcolor: "#f9fafb" } }}>
                    ...
                    <TableCell>{item.statusType || "ไม่ระบุ"}</TableCell>
                    <TableCell>
                      <PatientStatusIcons
                        isOpd={!!(item.isOpd || item.isAll)}
                        isIpd={!!(item.isIpd || item.isAll)}
      ...
      >
        ...
        >
          <Typography variant="h6" sx={{ fontWeight: 600 }}>เพิ่มข้อมูลเหตุผล</Typography>
          <IconButton size="small" onClick={() => setAddModalOpen(false)} sx={{ color: "#fff" }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <DialogContent sx={{ p: 3 }}>
          {renderReasonFormFields(addForm, setAddForm, addErrors, handleAddZoneChange, handleAddTypeChange)}
        </DialogContent>
      ...
      >
        ...
        >
          <Typography variant="h6" sx={{ fontWeight: 600 }}>แก้ไขข้อมูลเหตุผล</Typography>
          <IconButton size="small" onClick={() => setEditModalOpen(false)} sx={{ color: "#fff" }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <DialogContent sx={{ p: 3 }}>
          {editForm && renderReasonFormFields(editForm, setEditForm, editErrors, handleEditZoneChange, handleEditTypeChange)}
        </DialogContent>
        <DialogActions sx={{ px: 3, py: 2, justifyContent: "space-between" }}>
          <Button
            variant="outlined"
