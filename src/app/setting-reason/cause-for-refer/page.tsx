...
import { useReferralCauseStore } from "@/stores/referralCauseStore";
...
const getCountBadgeSx = (count: number) => {
  if (count >= 50)
    return { bgcolor: "#dcfce7", color: "#166534", borderRadius: "9999px", px: 1.2, py: 0.2, fontSize: "0.8rem", fontWeight: 500, whiteSpace: "nowrap" };
  if (count >= 20)
    return { bgcolor: "#fef9c3", color: "#4b5563", borderRadius: "9999px", px: 1.2, py: 0.2, fontSize: "0.8rem", fontWeight: 500, whiteSpace: "nowrap" };
  if (count >= 5)
    return { bgcolor: "#dbeafe", color: "#1d4ed8", borderRadius: "9999px", px: 1.2, py: 0.2, fontSize: "0.8rem", fontWeight: 500, whiteSpace: "nowrap" };
  return { bgcolor: "#f3f4f6", color: "#4b5563", borderRadius: "9999px", px: 1.2, py: 0.2, fontSize: "0.8rem", fontWeight: 500, whiteSpace: "nowrap" };
};

/* ── ReferralType options (ใช้สำหรับสถานะ / ใช้สำหรับผู้ป่วย in modal) matching Nuxt enum ── */
const referralTypeOptions = [
  { value: "Refer Out - ส่งตัวออก", name: "Refer Out - ส่งตัวออก" },
  { value: "Refer In - รับตัวเข้า", name: "Refer In - รับตัวเข้า" },
  { value: "Refer Back - ส่งตัวกลับ", name: "Refer Back - ส่งตัวกลับ" },
  { value: "Request Refer - ร้องขอส่งตัว", name: "Request Refer - ร้องขอส่งตัว" },
  { value: "ทุกประเภท", name: "ทุกประเภท" },
];
...
export default function CauseForReferPage() {
  const causeStore = useReferralCauseStore();
  ...
  const doFetch = useCallback(async (
    ...
    try {
      ...
      const res = await causeStore.findAndCountReferralCause(params);
      // API may return array or { referralCauses, totalCount }
      if (Array.isArray(res)) {
        setItems(res);
        setTotalCount(res.length);
      } else if (res?.referralCauses) {
        setItems(res.referralCauses);
      ...
      setError("ไม่สามารถโหลดข้อมูลสาเหตุการส่งตัวได้");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const refetch = useCallback(() => {
    doFetch(page, rowsPerPage, search, selectedZone, selectedType, selectedHospital, selectedPatientType, patientFilters);
  }, [page, rowsPerPage, search, selectedZone, selectedType, selectedHospital, selectedPatientType, patientFilters]);

  useEffect(() => {
    refetch();
  }, [page, rowsPerPage, selectedZone, selectedType, selectedHospital, selectedPatientType, patientFilters]);
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimerRef.current = setTimeout(() => {
      setPage(1);
      doFetch(1, rowsPerPage, value, selectedZone, selectedType, selectedHospital, selectedPatientType, patientFilters);
    }, 800);
  };
  ...
  const handleToggleActive = async (item: any) => {
    ...
    try {
      await causeStore.updateReferralCause(item.id, { isActive: !item.isActive });
    } catch (err) {
  ...
  const validateAddForm = () => {
    ...
    if (!addForm.name.trim()) errs.name = "กรุณากรอกชื่อสาเหตุ";
    if (!addForm.patientType) errs.patientType = "กรุณาเลือกใช้สำหรับผู้ป่วย";
  ...
  const handleAddSave = async () => {
    ...
    try {
      ...
      const data: any = {
        ...
        referralType: addForm.patientType,
        isOpd: addForm.isOpd,
        isIpd: addForm.isIpd,
        isEr: addForm.isEr,
      };
      ...
      await causeStore.createReferralCause(data);
      ...
      if (msg.toLowerCase().includes("already exists")) {
        setAddErrors({ name: "ชื่อสาเหตุนี้มีอยู่แล้ว" });
      }
    } finally {
  ...
  const handleOpenEdit = (item: any) => {
    setEditForm({
      id: item.id,
      name: item.name || "",
      patientType: item.referralType || "",
      isOpd: !!(item.isOpd || item.isAll),
      isIpd: !!(item.isIpd || item.isAll),
      isEr: !!(item.isEr || item.isAll),
  ...
  const validateEditForm = () => {
    ...
    const errs: Record<string, string> = {};
    if (!editForm.name.trim()) errs.name = "กรุณากรอกชื่อสาเหตุ";
    if (!editForm.patientType) errs.patientType = "กรุณาเลือกใช้สำหรับผู้ป่วย";
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const data: any = {
        ...
        referralType: editForm.patientType,
      ...
      await causeStore.updateReferralCause(editForm.id, data);
  ...
  /* ════════════════════════════════════════════════════════
     DELETE MODAL (soft delete via update isDelete: true)
     ════════════════════════════════════════════════════════ */
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setModalLoading(true);
      await causeStore.updateReferralCause(deletingItem.id, { isDelete: true });
  ...
  const renderCauseFormFields = (
    ...
    <>
      {/* Section: ใช้สำหรับสถานพยาบาล (zone/type/hospital) */}
      <Box sx={{ border: "1px solid #e5e7eb", borderRadius: 2, mb: 2 }}>
        <SectionHeader title="ใช้สำหรับสถานพยาบาล" />
      ...
      {/* Section: ข้อมูลสาเหตุการส่งต่อ */}
      <Box sx={{ border: "1px solid #e5e7eb", borderRadius: 2 }}>
        <SectionHeader title="ข้อมูลสาเหตุการส่งต่อ" />
        <Box sx={{ p: 2.5, display: "flex", flexDirection: "column", gap: 2 }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr 1fr" }, gap: 2 }}>
            {/* ชื่อสาเหตุ */}
            <Box>
              <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>
                ชื่อสาเหตุ <span style={{ color: "red" }}>*</span>
              </Typography>
              <TextField
                fullWidth
                size="small"
                placeholder="ตั้งชื่อสาเหตุ"
                value={form.name}
            ...
            {/* ใช้สำหรับผู้ป่วย (ReferralType dropdown) */}
            <Box>
              <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>
              ...
              <Select
                ...
                onChange={(e) => setForm((prev: any) => ({ ...prev, patientType: e.target.value }))}
                sx={selectSx}
                MenuProps={dropdownMenuProps}
                error={!!errors.patientType}
              >
                <MenuItem value="">
                  <em style={{ color: "#9ca3af" }}>เลือกประเภทผู้ป่วย</em>
                </MenuItem>
              ...
              {errors.patientType && <Typography color="error" variant="caption">{errors.patientType}</Typography>}
            </Box>
            {/* ใช้กับประเภท (OPD/IPD/ER checkboxes) */}
            <Box>
              <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>
                ใช้กับประเภท <span style={{ color: "red" }}>*</span>
              </Typography>
              <FormGroup row>
                <FormControlLabel
                  control={
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ bgcolor: "#fff", border: "1px solid #e5e7eb", borderRadius: 2, p: 2.5, mb: 0 }}>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <TextField
            fullWidth
            size="small"
            placeholder="ค้นหาสาเหตุการส่งต่อ"
            value={search}
        ...
        {/* Filters row 1: Zone, Type, Hospital, ใช้สำหรับสถานะ */}
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr", lg: "1fr 1fr 1fr 1fr" }, gap: 2, mb: 2 }}>
          {/* Zone */}
          <Box>
            <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>โซน</Typography>
            <Select
              fullWidth
              size="small"
              displayEmpty
          ...
          {/* ใช้สำหรับสถานะ (ReferralType) */}
          <Box>
            <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>ใช้สำหรับสถานะ</Typography>
            <Select
              fullWidth
              size="small"
              displayEmpty
              value={selectedPatientType}
              onChange={(e) => { setSelectedPatientType(e.target.value); setPage(1); }}
              sx={selectSx}
              MenuProps={dropdownMenuProps}
            >
              <MenuItem value="">
                <em style={{ color: "#9ca3af" }}>เลือกสถานะ</em>
              </MenuItem>
              {referralTypeOptions.map((o) => (
                <MenuItem key={o.value} value={o.value}>
                  {o.name}
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 2 }}>
          <Box>
            <Typography variant="body2" sx={{ mb: 0.5, fontWeight: 500 }}>สำหรับผู้ป่วย</Typography>
            <FormGroup row>
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: "#036245" }}>
                {["ลำดับ", "ชื่อสาเหตุที่ส่ง Refer out", "สถานพยาบาล", "ใช้กับประเภท", "สำหรับผู้ป่วย", "สถานะใช้งาน", "จัดการข้อมูล"].map(
                  (header) => (
              ...
              {loading ? (
                ...
                items.map((item, idx) => (
                  <TableRow key={item.id} hover sx={{ "&:nth-of-type(even)": { bgcolor: "#f9fafb" } }}>
                    ...
                    <TableCell>{item.hospital?.name || "ไม่ระบุ"}</TableCell>
                    <TableCell>{item.referralType || "ไม่ระบุ"}</TableCell>
                    <TableCell>
                      <PatientStatusIcons
                        isOpd={!!(item.isOpd || item.isAll)}
                        isIpd={!!(item.isIpd || item.isAll)}
      ...
      >
        ...
        >
          <Typography variant="h6" sx={{ fontWeight: 600 }}>เพิ่มข้อมูลสาเหตุการส่งตัว</Typography>
          <IconButton size="small" onClick={() => setAddModalOpen(false)} sx={{ color: "#fff" }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <DialogContent sx={{ p: 3 }}>
          {renderCauseFormFields(addForm, setAddForm, addErrors, handleAddZoneChange, handleAddTypeChange)}
        </DialogContent>
      ...
      >
        ...
        >
          <Typography variant="h6" sx={{ fontWeight: 600 }}>แก้ไขข้อมูลสาเหตุการส่งตัว</Typography>
          <IconButton size="small" onClick={() => setEditModalOpen(false)} sx={{ color: "#fff" }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <DialogContent sx={{ p: 3 }}>
          {editForm && renderCauseFormFields(editForm, setEditForm, editErrors, handleEditZoneChange, handleEditTypeChange)}
        </DialogContent>
        <DialogActions sx={{ px: 3, py: 2, justifyContent: "space-between" }}>
          <Button
            variant="outlined"
