...
/* ------------------------------------------------------------------ */
/*  IPD Inner (needs useSearchParams in Suspense)                      */
/* ------------------------------------------------------------------ */
function IPDReferralInner() {
  ...
  useEffect(() => {
    if (draftId) {
      findOneReferral(draftId).then((res: any) => {
        ...
        if (!doc || !doc.data) {
          console.warn("[IPD Draft] No valid document found in response");
          return;
        }
        setReferInfo(doc);
        ...
        const draftFormData: Record<string, any> = {
          ...
          // Inject draft doctor/cause references so RequestReferralForm can use them
        ...
        console.error("[IPD Draft] Error loading draft:", err);
      });
    }
  ...
  // Get referralType number from kind string
  const getReferralTypeNumber = useCallback((kindStr: string) => {
    switch (kindStr) {
      ...
      case "referIPD": return 4;
      case "REFER_IN": return 4;
      default: return 0;
    }
  }, []);

  // Get fromHospital based on role
  ...
  // Parse toManyHospital from parsedHospitals
  ...
  // Build appointmentData from query params
  ...
  const handleSave = useCallback(
    async (saveType: "draft" | "submit") => {
      ...
      try {
        ...
        // Build payload matching Nuxt's IPD form structure
        const payload: Record<string, any> = {
          ...
          fromHospital,
          toManyHospital,
          referralKind: 4, // 4 = IPD (differs from ER which is 1)
        ...
        console.log("[IPD Save] payload:", JSON.stringify(payload, null, 2));
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      {/* Step 2: Referral Form — reuse RequestReferralForm (same as ER) */}
      {hasHospital && (
        <Box sx={{ mt: 3 }}>
