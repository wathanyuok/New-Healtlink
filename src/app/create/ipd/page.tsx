...
/*  IPD Inner                                                          */
/* ------------------------------------------------------------------ */
function IPDReferralInner() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const kind = searchParams.get("kind") || "referIPD";
  ...
  const parsedHospitals: SelectedHospital[] = useMemo(() => {
    ...
    try { return JSON.parse(hospitalsParam); } catch { return []; }
  }, [hospitalsParam]);
  ...
  useEffect(() => {
    if (groupCase) { findGroupCaseFn(groupCase).catch(console.error); }
  }, [groupCase, findGroupCaseFn]);

  const title = "ส่งตัวผู้ป่วยใน (IPD)";

  const navigateBack = useCallback(() => {
    if (hasHospital) {
      router.push(`/create/ipd?kind=${kind}`);
    } else {
      router.push("/create");
    }
  ...
  const handleHospitalNext = useCallback(
    (hospitals: SelectedHospital[]) => {
      ...
      router.push(`/create/ipd?${query.toString()}`);
    },
    [router, kind, draftId]
  );
  ...
  const handleSave = useCallback(
    async (saveType: "draft" | "submit") => {
      ...
      try {
        if (saveType === "submit") {
          const requiredFields = ["patient_pid", "visit_primary_symptom_main_symptom", "visit_primary_symptom_current_illness", "pe", "Imp"];
          const missing = requiredFields.filter((f) => !formData[f]);
          if (missing.length > 0) { alert("กรุณาตรวจสอบข้อมูลและกรอกให้ครบถ้วน"); return; }
        } else {
          if (!formData.patient_pid) { alert("กรุณากรอกเลขบัตรประชาชน"); return; }
        }

        const payload = { ...formData, referralType: kind, hospitals: parsedHospitals, saveType };
    ...
    [isLoading, sendData, formData, parsedHospitals, isDraftStatus, referInfo, kind, createReferralDocument, updateReferralDocument, router]
  );
  ...
  const ActionButtons = () => (
    <Stack direction="row" spacing={1.5} flexWrap="wrap" useFlexGap>
      {hasHospital && (
        <>
          <Button variant="contained" startIcon={<ListIcon />} onClick={() => setViewHospitalsOpen(true)} disabled={isLoading}
            sx={{ bgcolor: "#3b82f6", "&:hover": { bgcolor: "#091b63" }, textTransform: "none" }}>
          ...
          {isDraftStatus && referInfo?.referralStatus?.name === "ฉบับร่าง" && (
            <>
              <Button variant="outlined" startIcon={<SaveIcon />} onClick={() => handleSave("draft")} disabled={isLoading || sendData} sx={{ textTransform: "none" }}>
                {isLoading ? "กำลังบันทึก..." : "บันทึกและแก้ไขฉบับร่าง"}
              </Button>
              <Button variant="contained" startIcon={<SendIcon />} onClick={() => handleSave("submit")} disabled={isLoading || sendData}
                sx={{ bgcolor: "#00AF75", "&:hover": { bgcolor: "#036245" }, textTransform: "none" }}>
          ...
          {isDraftStatus && referInfo?.referralStatus?.name === "รอตอบรับ" && (
            <Button variant="outlined" startIcon={<SaveIcon />} onClick={() => handleSave("submit")} disabled={isLoading || sendData} sx={{ textTransform: "none" }}>
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <BreadcrumbsRefer basePath="/create/ipd" kind={kind} />

      {/* Step 1: Hospital Selection (same as ER - multi-select with branches) */}
      {!hasHospital && (
      ...
      {hasHospital && (
        <Box sx={{ mt: 3 }}>
          <Paper sx={{ p: 3, borderRadius: 2 }}>
            <IPDReferralForm kind={kind} hospitals={parsedHospitals} isDraft={isDraftStatus} referInfo={referInfo} formData={formData} onUpdate={updateFormData} />
          </Paper>
      ...
      <ViewSelectedHospitalsModal open={viewHospitalsOpen} onClose={() => setViewHospitalsOpen(false)} hospitals={parsedHospitals} />
    </Box>
  );
}
...
interface IPDFormProps { kind: string; hospitals: SelectedHospital[]; isDraft: boolean; referInfo: any; formData: Record<string, any>; onUpdate: (partial: Record<string, any>) => void; }

function IPDReferralForm({ kind, hospitals, isDraft, referInfo, formData, onUpdate }: IPDFormProps) {
  return (
    <Box>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr 1fr" }, gap: 2, mb: 3 }}>
        ...
        <FormField label="ระดับ ICU" value={formData.icuLevel || ""} onChange={(v) => onUpdate({ icuLevel: v })} />
      </Box>
      ...
      <Box sx={{ mb: 3 }}>
        <FormField label="การรักษา" value={formData.treatment || ""} onChange={(v) => onUpdate({ treatment: v })} multiline rows={3} />
      </Box>
    </Box>
  );
}
...
/* ---- Page ---- */
export default function IPDReferralPage() {
  return (
