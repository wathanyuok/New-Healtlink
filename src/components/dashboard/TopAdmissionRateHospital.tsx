...
export default function TopAdmissionRateHospital({ role, dashboardType }: Props) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const response = await store.getTop5HighestAcceptingHospitals(params);
  ...
  const tableData = useMemo(() => {
    ...
    return apiData.map((item: any) => ({
      ...
      percentage: (item.acceptanceRate || "0").toString().replace("%", ""),
  ...
  const downloadExcel = async () => {
    try {
      ...
      const response = await store.exportTop5HighestAcceptingHospitals(params);
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Top5 โรงพยาบาลอันดับสูง");
      XLSX.writeFile(wb, `Top5_สถานพยาบาลอัตราการรับสูงสุด_${Date.now()}.xlsx`);
    } catch (error: any) {
  ...
  return (
    <Paper sx={{ borderRadius: 3, overflow: "hidden" }}>
      ...
      <FilterSection filters={filters} onFilterChange={setFilters} onClear={clearFilters} hideHospitalFilter />
            ...
            <Table size="small">
              <TableHead sx={{ bgcolor: "#036245" }}>
                <TableRow>
                  ...
                  <TableCell sx={{ color: "#fff", fontWeight: 600 }}>อัตราการรับ</TableCell>
                  <TableCell sx={{ color: "#fff", fontWeight: 600 }}>Refer In</TableCell>
                  <TableCell sx={{ color: "#fff", fontWeight: 600 }}>Refer Out</TableCell>
                  <TableCell sx={{ color: "#fff", fontWeight: 600 }}>Refer Back</TableCell>
              ...
              <TableBody>
                {tableData.map((row, idx) => (
                  <TableRow key={`hosp-${idx}`} hover>
                    ...
                    <TableCell>{row.referIn}</TableCell>
                    <TableCell>{row.referOut}</TableCell>
                    <TableCell>{row.referBack}</TableCell>
